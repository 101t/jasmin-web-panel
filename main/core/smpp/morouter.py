...
class MORouter(object):
	...
	def create(self, data):
		...
		if rtype == 'randomroundrobinmoroute':
			if len(connectors) < 2:
				raise MutipleValuesRequiredKeyError('Round Robin route requires at least two connectors')
			ikeys['connectors'] = ';'.join(connectors)
		elif rtype == 'failovermoroute':
			if len(connectors) < 2:
				raise MutipleValuesRequiredKeyError('FailOver route requires at least two connectors')
			ikeys['connectors'] = ';'.join(connectors)
		else:
			if len(connectors) != 1:
